...
setwd("C:/Users/jonas/Nextcloud/Masterarbeit/ps - Kopie")
ps.name = "Militarization-of-US-police"; sol.file = paste0(ps.name,"_sol.Rmd")
libs = c("dplyr", "kableExtra", "ggplot2", "psych", "fixest", "modelsummary", "tidyr", "fwildclusterboot") # character vector of all packages you load in the problem set

#name.rmd.chunks(sol.file)
create.ps(sol.file=sol.file, ps.name=ps.name, libs=libs, extra.code.file = "custom_functions.R", addons = "quiz")
...
#< quiz "problemset_focus" 
question: What will you analyze in this problem set? 
sc:
- The cost-efficiency of military equipment in law enforcement.
- The effectiveness of police training programs in handling military-grade equipment.
- The empirical effects of the police militarization on policing outcomes.*
- The comparison of military equipment usage in the U.S. versus other countries. 
success: Correct! This problem set focuses on analyzing the empirical effects of the 1033 Program. 
failure: Try again. 
#>
...
#< quiz "criticism_1033_program" 
question: What is a major criticism of the 1033 Program? 
sc:

- It fails to provide enough military equipment to law enforcement.
- It fosters an adversarial relationship between police and communities, potentially escalating violence.*
- It does not adequately train officers in using military equipment.
- It is too costly for local law enforcement agencies. 

success: Correct.
failure: Try again. Consider how the presence of military equipment might affect police-community relations. 
#>
...
The `readRDS()` function in R allows us to read and save data in the `.RDS` format, a common file type for storing R objects. Here are the steps to load your data:

1. **Calling the `readRDS()` Function**: To load a dataset, call the `readRDS()` function with the path to your file enclosed in quotes. Ensure you use forward slashes (`/`) instead of backslashes (`\`) if you copy the file path from your file explorer, as R does not recognize backslashes.

Example:

```{r "2.1.i.1", eval=FALSE}
readRDS("path_to_file/file_name.RDS")
```

2. **Assigning the Data to a Variable**: After loading the data, assign it to a variable for easy reference in your analysis. You can use either `=` or `<-` for assignment.

Example:

```{r "2.1.i.2", eval=FALSE}
dat <- readRDS("path_to_file/file_name.RDS")
dat = readRDS("path_to_file/file_name.RDS")
```
...
mil_years <- mil |>
  group_by(year) |>
  summarise(`total_volume` = sum(aid_value, na.rm = TRUE),
            `weapons_volume` = sum(weapons_value, na.rm = TRUE),
            `vehicles_volume` = sum(vehicles_value, na.rm = TRUE),
            `gears_volume` = sum(gears_value, na.rm = TRUE),
            `other_volume` = sum(other_equipment_value, na.rm = TRUE)) |>
  pivot_longer(!year, names_to = "aid_type", values_to = "aid_value") |>
...
ggplot(data = mil_years, mapping = aes(x = year, y = aid_value, colour = aid_type)) +
  geom_line(linewidth = 1.2) +
  scale_y_continuous(breaks = c(0,1e8,2e8,3e8,4e8,5e8),
                     labels = c("0","100M","200M","300M","400M","500M")) +
  labs(y = "Aid value (USD)", x = "", title = "Volume of received military aid over time") +
...
mil_recieving <- mil |>
  mutate(hi_low = ifelse(recieving_years >= 3, "High recipients", "Low recipients")) |>
  group_by(hi_low, year) |>
  summarise(crime_rate = mean(crime_rate),
            aid_value = mean(lag_log_aid_value))
...
mod_simple <- feols(crime_rate ~ lag_log_aid_value, data = mil)
#< task
...
#< quiz "coeffiecient_interpretation_3"
question: How can we interpret the coefficient of the lagged, logarithmized  military aid in the regression if we assume a causal relationship?
sc:
- 10$ more aid leads to approximately 7.5 more crimes.
- 10% more aid leads to approximately 7.5 less crimes.
- 10$ more aid leads to approximately 0.75% more crimes.
- 10% more aid leads to approximately 7.5 more crimes per 100,000 population.*
success: Great, your answer is correct! 
failure: Try again. Remember we use the crime rate as dependent variable.
#>
...
mod_pop <- feols(crime_rate ~ lag_log_aid_value + log_population, data = mil)
...
mod_county <- feols(crime_rate ~ lag_log_aid_value + log_median_income + poverty_rate + unempl_rate + log_population + share_male + share_black + share_15_19_years + share_20_24_years + share_25_29_years + share_30_34_years | county, data = mil)
...
mod_cluster <-  feols(crime_rate ~ lag_log_aid_value + log_median_income + poverty_rate + unempl_rate + log_population + share_male + share_black + share_15_19_years + share_20_24_years + share_25_29_years + share_30_34_years | county + state_year_interaction, data = mil, cluster = ~ state)


modelsummary(list("Conventional" = mod_fe, "CRVE" = mod_cluster), 
             output = "kableExtra",
             coef_map = c("lag_log_aid_value"),
             gof_map = c("nobs"),
             stars = TRUE
             ) |>
  kable_classic(full_width = TRUE) |>
...
Running thousands of full regressions can be the main bottleneck in practice.  
For linear-in-parameters models (OLS, IV, linear probability, Poisson PPML, etc.) we can exploit simple algebra to avoid that expense.

1. **Pre-compute once**

   - $\hat{\beta}$ – the original coefficient vector  
   - $Q = (X^{\top}X)^{-1}X^{\top}$ – sometimes called the *cross-product operator*  

   These objects stay the same across all bootstrap draws.

2. **Draw cluster weights only**  

   For each bootstrap iteration $b$ generate one Rademacher/Webb weight  
   $w_g^{(b)}$ per cluster $g = 1,\dots,G$.

3. **Update coefficients with a single matrix multiplication**

   The bootstrap residual vector is  

$$
\tilde u^{(b)} =\ w^{(b)} \circ \hat u,
$$

   and the updated coefficient estimate follows immediately  

$$
\hat{\beta}^{(b)} =\ \hat{\beta} + Q \tilde u^{(b)}.
$$

   No solver, no iterated least squares — just one `crossprod()` call in R.

4. **Form the bootstrap test statistic**

   Combine $\hat{\beta}^{(b)}$ with the pre-computed cluster-robust variance matrix  
   (optionally re-scaled for **WCR**) to obtain  

$$
t^{(b)} = \frac{\hat{\beta}_j^{(b)} - \theta_0}{\widehat{\mathrm{SE}}(\hat{\beta}_j^{(b)})}
$$

   Because step 3 needs only $\mathcal{O}(KN)$ operations (where $K$ is the number of regressors and $N$ the sample size), a few thousand draws finish in seconds even with many fixed effects.
...
```{r}
reg_ols_simple <- feols(crime_rate ~ mil_aid, data = dat)
reg_ols_control <- feols(crime_rate ~ mil_aid + crime_experience, data = dat)
...
```{r}
reg_first_stage <- feols(mil_aid ~ mil_ex, data = dat)
dat$fit_mil_aid <- fitted(reg_first_stage)
...
```{r}
reg_second_stage <- feols(crime_rate ~ fit_mil_aid, data = dat)
...
```{r}
reg_iv <- feols(crime_rate ~ 1 | mil_aid ~ mil_ex, data = dat)
...
#>
mod_first_stage <- feols(lag_log_aid_value ~ milex_iv + log_median_income + poverty_rate + unempl_rate + log_population + share_male + share_black + share_15_19_years + share_20_24_years + share_25_29_years + share_30_34_years | county + state_year_interaction, data = mil, cluster = ~ state)
...
#>
mod_iv <- feols(crime_rate ~ log_median_income + poverty_rate + unempl_rate + log_population + share_male + share_black + share_15_19_years + share_20_24_years + share_25_29_years + share_30_34_years | county + state_year_interaction | lag_log_aid_value ~ milex_iv, data = mil, cluster = ~ state, ssc = ssc(adj=FALSE, cluster.adj = FALSE, t.df = "conventional"))
...
#< quiz "coeffiecient_interpretation_2"
question: If we assume causality, how would we interpret the coefficient of the IV regression of -59.293?
sc:
- 10$ more aid leads to approximately 5.9 more crimes.
- 10% more aid leads to approximately 59 less crimes per 100,000 population.
- 10$ more aid leads to approximately 5.9% less crimes.
- 10% more aid leads to approximately 5.9 less crimes per 100,000 population.*
success: Great, your answer is correct! 
failure: Try again. Remember we use the crime rate as dependent variable and log aid as explanatory variable.
#>
...
#< quiz "crime_type_effect"
question: For which crime type does military aid show the most substantial negative effect?
sc:
- Murder
- Robbery
- Larceny*
- Assault
success: Correct! Military aid has the most substantial negative effect on larceny.
failure: Try again. Focus on the magnitude and significance of the coefficients.
#>
...
#< quiz "deterrence_mechanisms"
question: Which of the following best describes the two main ways military aid might deter crime?
sc:
- Fear of military equipment and improved police effectiveness.*
- Increased police salaries and longer patrol hours.
- Community engagement programs and educational campaigns.
- Randomized policing strategies and harsher sentencing laws.
success: Correct! Military aid can deter crime through fear of militarized equipment and by enhancing police effectiveness.
failure: Try again. Think about how military aid affects both the psychological and operational aspects of policing.
#>
...
mil <- mil |>
  mutate(murder_arrests_per_crime = arrest_rate_murder/murder_rate,
         robbery_arrests_per_crime = arrest_rate_robbery/robbery_rate,
         assault_arrests_per_crime = arrest_rate_assault/assault_rate,
         burglary_arrests_per_crime = arrest_rate_burglary/burglary_rate,
         larceny_arrests_per_crime = arrest_rate_larceny/larceny_rate,
         vehicle_theft_arrests_per_crime = arrest_rate_vehicle_theft/vehicle_theft_rate)
...
#< quiz "interpretation_coefficient_relative_arrest_rate"
question: If we assume a causal relationship, what would the coefficient for murder of -0.070 mean? If we assume that every murder is committed by one single person.
sc:
- A 1% increase in military aid leads to a 7 percent lower arrest probability for the murder.
- A 100% increase in military aid leads to a 7 percentage point lower arrest probability for the murder.*
- A 10% increase in military aid leads to a 7 percent lower arrest probability for the murder.
- A 100% increase in military aid leads to a 7 percent lower arrest probability for the murder.
success: Correct! 
failure: Try again.
#>
...
#< quiz "coeffiecient_interpretation_4"
question: How can we interpret the coefficient of the lagged, logarithmized other equipment aid in the regression on crime rate if we assume a causal relationship?
sc:
- 10% more aid leads to approximately 7 less crimes per 100,000 population.*
- 10$ more aid leads to approximately 7 more crimes per 100,000 population.
- 10% more aid leads to approximately 70 less crimes per 100,000 population.
- 10$ more aid leads to approximately 7% less crimes per 100,000 population.
success: Great, your answer is correct! 
failure: Try again.
#>
